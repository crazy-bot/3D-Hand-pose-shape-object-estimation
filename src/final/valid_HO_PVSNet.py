...
parser.add_argument('--saveJSON', type=bool, default=True, help='save json or not')
parser.add_argument('--handout', type=str, default='hand_pvsnet_v6_{}.json'.format(validset), help='File to save the predictions.')
parser.add_argument('--objout', type=str, default='obj_pvsnet_v6_{}.json'.format(validset), help='File to save the predictions.')
parser.add_argument('--figMode', default='', choices=['2D', '3DPC', '3DMesh', ''])
parser.add_argument('--multi_GPU', type=bool, default=True, help='If the model to load was trained in multi-GPU')
args = parser.parse_args()
#######################################################################################
###################### Model, criterion and optimizer ###########################
print('==> Constructing model ... HO_PVSNet')
pvsnet = HO_PVSNet()
if (args.multi_GPU):
    pvsnet = torch.nn.DataParallel(pvsnet)
pvsnet = pvsnet.to(device, dtype)
pvsnet.load_state_dict(torch.load(PVSNet_ckpt)['model_state_dict'])
...
with open(file_path) as tf:
    ...
    for record in records:
        ...
        if (validset != 'evaluation'):
            ...
            ################ for evaluation set ############
            mano = load_model(MANO_MODEL_PATH, ncomps=6, flat_hand_mean=True)
            objCorners = annot['objCorners3D']
        ...
        #     'refpoint': refpoint,
        # }
        # voxel88, heatmap_joints, heatmap_bbox, voxel44, mesh_voxel, norm_handmesh, norm_objmesh = transform.train_transform(sample)
        # # testVis(mesh_voxel, heatmap_joints, heatmap_bbox,norm_handmesh, norm_objmesh)
        ...
        if (args.figMode != ''):
            ...
            if (validset == 'evaluation'):
                if (args.figMode == '2D'):
                    ...
                    plot3DPCforTest(outUVD, fileName)
                elif (args.figMode == '3DMesh'):
                    plot3DMeshforTest(mano, objMesh, handmeshUVD, objmeshUVD, fileName)
            else:
                gtUVD = {
