...
class HO_Posenet(nn.Module):
    def __init__(self, input_channels, hand_channels, obj_channels):
        super(HO_Posenet, self).__init__()

        self.front_layers = nn.Sequential(
            Basic3DBlock(input_channels, 16, 7),  # (b,16,88,88,88)
            Pool3DBlock(2),  # (b,16,44,44,44)
            Res3DBlock(16, 32),  # (b,32,44,44,44)
            Res3DBlock(32, 32),  # (b,32,44,44,44)
            Res3DBlock(32, 32)  # (b,32,44,44,44)
        )
        ...
        self.back_layers = nn.Sequential(
            Res3DBlock(32, 32),  # (b,32,44,44,44)
            Basic3DBlock(32, 32, 1),  # (b,32,44,44,44)
            Basic3DBlock(32, 32, 1),  # (b,32,44,44,44)
        )

        self.handlayer = nn.Conv3d(32, hand_channels, kernel_size=1, stride=1, padding=0)  # (b,21,44,44,44)
        self.objlayer = nn.Conv3d(32, obj_channels, kernel_size=1, stride=1, padding=0)  # (b,8,44,44,44)
        self._initialize_weights()

    def forward(self, x):
        x = self.front_layers(x) #  (b,32,44,44,44)
        x = self.encoder_decoder(x) #  (b,32,44,44,44)
        x = self.back_layers(x)     #  (b,32,44,44,44)
        hand_hmp = self.handlayer(x)  #  (b,21,44,44,44)
        obj_hmp = self.objlayer(x)
        result = {
            'handpose': hand_hmp,
            'objpose': obj_hmp
        }
