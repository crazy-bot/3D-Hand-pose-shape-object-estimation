...
from src.train_helper import train_HO_PVSNetv6, val_HO_PVSNetv6
...
ap.add_argument("--save_ckpt", type=str, help="relative path to save checkpoint", default='HO_PVSNet')
...
if args.load_ckpt is not '':
    ...
    voxelnet_weights = {k.replace('voxelNet.', ''): hopv_weights[k] for k in list(hopv_weights.keys()) if
                        'voxelNet' in k}
    shapenet_weights = torch.load(SNet_ckpt)['model_state_dict']
...
for epoch in range(start_epoch, start_epoch + epochs_num):
    ...
    net.train()
    train_HO_PVSNetv6( net, criterion1, optimizer, train_loader, device=device)
    net.eval()
    val_HO_PVSNetv6(net, val_loader, device=device)
    ########### take schedular step #########
    scheduler.step()
