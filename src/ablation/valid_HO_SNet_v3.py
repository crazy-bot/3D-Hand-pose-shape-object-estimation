...
from dataset.HO_Data.HO_shape_v3 import *
...
parser.add_argument('--handout', type=str, default='hand_snetv3_valid.json', help='File to save the predictions.')
parser.add_argument('--objout', type=str, default='obj_snetv3_valid.json', help='File to save the predictions.')
...
#######################################################################################
###################### Model weights load ###########################
print('==> Constructing model ... HO_SNet_v3')
snetv3 = HO_ShapeNet(input_channels=1 + 21 + 8 + 1, hand_channels=no_handverts, obj_channels=no_objverts)
if(args.multi_GPU):
    snetv3 = torch.nn.DataParallel(snetv3)
snetv3 = snetv3.to(device, dtype)
snetv3.load_state_dict(torch.load(SNet_v3_ckpt)['model_state_dict'])
...
############### savefig dir ###########
if (args.figMode != ''):
    saveFolder = SAVEFIG_DIR + '/SNet_v3/'
...
with open(file_path) as tf:
    ...
    for record in records:
        ...
        depth_voxel, heatmap_joints, heatmap_bbox, mesh_voxel, norm_handmesh, norm_objmesh = transform.train_transform(
            sample)
