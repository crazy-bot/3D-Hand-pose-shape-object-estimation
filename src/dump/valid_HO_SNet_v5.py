...
from dataset.HO_Data.HO_shape_v5 import *
...
#######################################################################################
###################### Model weights load ###########################
print('==> Constructing model ... HO_SNet_v5')
snetv5 = HO_ShapeNet(input_channels=1, hand_channels=no_handverts, obj_channels=no_objverts)
if(args.multi_GPU):
    snetv5 = torch.nn.DataParallel(snetv5)
snetv5 = snetv5.to(device, dtype)
snetv5.load_state_dict(torch.load(SNet_v5_ckpt)['model_state_dict'])
...
############### savefig dir ###########
if (args.figMode != ''):
    saveFolder = SAVEFIG_DIR + '/SNet_v5/'
...
with open(file_path) as tf:
    ...
    for record in records:
        ...
        depth_voxel, mesh_voxel, norm_handmesh, norm_objmesh = transform.train_transform(sample)
        dm_voxel = np.zeros(depth_voxel.shape)
        dm_voxel[depth_voxel == 1] = 1
        dm_voxel[mesh_voxel == 1] = 1
        #testVis(depth_voxel,norm_handmesh, norm_objmesh)
        dm_voxel = torch.from_numpy(dm_voxel.reshape((1, 1, *dm_voxel.shape))).to(device, dtype)
        # norm_handmesh = torch.from_numpy(norm_handmesh.reshape(1, *norm_handmesh.shape)).to(device, dtype)
        # norm_objmesh = torch.from_numpy(norm_objmesh.reshape(1, *norm_objmesh.shape)).to(device, dtype)

        ####################### get prediction ############################

        with torch.no_grad():
            result = snetv5(dm_voxel)
