...
from dataset.HO_Data.HO_shape_v1 import *
...
parser.add_argument('--handout', type=str, default='hand_snetv1_{}.json'.format(validset), help='File to save the predictions.')
parser.add_argument('--objout', type=str, default='obj_snetv1_{}.json'.format(validset), help='File to save the predictions.')
...
#######################################################################################
###################### Model weights load ###########################
print('==> Constructing model ... HO_SNet_v1')
snetv1 = HO_ShapeNet(input_channels=1,hand_channels=no_handverts,obj_channels=no_objverts)
if(args.multi_GPU):
    snetv1 = torch.nn.DataParallel(snetv1)
snetv1 = snetv1.to(device, dtype)
snetv1.load_state_dict(torch.load(SNet_v1_ckpt)['model_state_dict'])
...
############### savefig dir ###########
if (args.figMode != ''):
    saveFolder = SAVEFIG_DIR + '/SNet_v1/'
...
with open(file_path) as tf:
    ...
    for record in records:
        ...
        voxel44, norm_handmesh, norm_objmesh = transform.train_transform(sample)
