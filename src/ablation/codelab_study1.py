...
def loadModel(device, dtype):
    print('==> Constructing model ..HO_PSNet')
    net = HO_PSNet()
    net = torch.nn.DataParallel(net)
    net = net.to(device, dtype)

    ################ load predefined checkpoint ###############
    net.load_state_dict(torch.load(PSNet_ckpt)['model_state_dict'])
    return net
...
if __name__ == '__main__':
    ...
    parser.add_argument('--handout', type=str, default='hand_PSNet.json', help='File to save the predictions.')
    parser.add_argument('--objout', type=str, default='obj_PSNet.json', help='File to save the predictions.')
