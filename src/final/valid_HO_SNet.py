...
#######################################################################################
###################### Model weights load ###########################
print('==> Constructing model ... HO_SNet')
snet = HO_ShapeNet(input_channels=1+21+8, hand_channels=no_handverts, obj_channels=no_objverts)
if(args.multi_GPU):
    snet = torch.nn.DataParallel(snet)
snet = snet.to(device, dtype)
snet.load_state_dict(torch.load(SNet_ckpt)['model_state_dict'])
