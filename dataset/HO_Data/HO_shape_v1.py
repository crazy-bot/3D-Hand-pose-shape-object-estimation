...
class Ho3DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (not self.isValid):
            ...
            depth_voxel,norm_handmesh,norm_objmesh = self.transform.train_transform(sample)
            ################ for testing purpose in visualization ###############
            if (self.preVis):
                self.testVis(depth_voxel,norm_handmesh,norm_objmesh)
            ...
            return (depth_voxel,norm_handmesh,norm_objmesh )
        else:
            pointcloud = Main_depthmap2points(depth, ux, uy, fx, fy)
            pointcloud = pointcloud.reshape(-1, 3)
            refpoint = Main_pixelToworld(com.reshape(1, -1), ux, uy, fx, fy)
