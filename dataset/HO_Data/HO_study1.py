...
class Ho3DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (not self.isValid):
            ...
            voxel88,heatmap_joints,heatmap_bbox,voxel44,norm_handmesh,norm_objmesh = self.transform.train_transform(sample)
            ################ for testing purpose in visualization ###############
            #print('norm_handmesh-- ',norm_handmesh)
            #print('norm_objmesh-- ', norm_objmesh)
            if (self.preVis):
                self.testVis(voxel88,heatmap_joints,heatmap_bbox,norm_handmesh,norm_objmesh)
            ...
            return (voxel88,heatmap_joints,heatmap_bbox,voxel44,norm_handmesh,norm_objmesh)
        else:
            pointcloud = Main_depthmap2points(depth, ux, uy, fx, fy)
            pointcloud = pointcloud.reshape(-1, 3)
            refpoint = Main_pixelToworld(com.reshape(1, -1), ux, uy, fx, fy)
    ...
    def testVis(self, voxel88,heatmap_joints,heatmap_bbox,norm_handmesh,norm_objmesh):
