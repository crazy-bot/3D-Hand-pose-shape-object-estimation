...
def loadModel(device, dtype):
    ...
    net.load_state_dict(torch.load(PVSNet_ckpt)['model_state_dict'])
    return net
...
def main(args):
    ...
    for idx in tqdm(range(db_size(set_name))):
        ...
        with torch.no_grad():
            ...
            out_handmesh = result['handverts'][0].cpu().numpy()
            handmeshUVD, handverts = pred2Org_mesh(out_handmesh, refpoint, ux, uy, fx, fy)

            ################# object conversion #######################
            obj_hmp = result['objpose'][0].cpu().numpy()
            objbboxUVD, objxyz = pred2Org_objbbox(obj_hmp, refpoint, ux, uy, fx, fy)

            out_objmesh = result['objverts'][0].cpu().numpy()
            objmeshUVD, objverts = pred2Org_mesh(out_objmesh, refpoint, ux, uy, fx, fy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--handout', type=str, default='hand_PVSNet.json', help='File to save the predictions.')
    parser.add_argument('--objout', type=str, default='obj_PVSNet.json', help='File to save the predictions.')
