...
from dataset.HO_Data.HO_shape_v6 import *
...
parser.add_argument('--handout', type=str, default='hand_snetv4_valid.json', help='File to save the predictions.')
parser.add_argument('--objout', type=str, default='obj_snetv4_valid.json', help='File to save the predictions.')
...
#######################################################################################
###################### Model weights load ###########################
print('==> Constructing model ... HO_SNet_v6')
snetv6 = HO_ShapeNet(input_channels=1+21+8, hand_channels=no_handverts, obj_channels=no_objverts)
if(args.multi_GPU):
    snetv6 = torch.nn.DataParallel(snetv6)
snetv6 = snetv6.to(device, dtype)
snetv6.load_state_dict(torch.load(SNet_v6_ckpt)['model_state_dict'])
...
############### savefig dir ###########
if (args.figMode != ''):
    saveFolder = SAVEFIG_DIR + '/SNet_v6/'
...
with open(file_path) as tf:
    ...
    for record in records:
        ...
        depth_voxel, heatmap_joints, heatmap_bbox, mesh_voxel,norm_handmesh,norm_objmesh  = transform.train_transform(sample)
        #dm_voxel = np.zeros(depth_voxel.shape)
        #dm_voxel[depth_voxel == 1] = 1
        depth_voxel[mesh_voxel == 1] = 1

        # joints = extract_coord_from_output(heatmap_joints)
        # objCorners = extract_coord_from_output(heatmap_bbox)
        # testVis2(depth_voxel,joints, objCorners)

        dm_voxel = torch.from_numpy(depth_voxel.reshape((1, 1, *depth_voxel.shape))).to(device, dtype)
        ...
        inputs = torch.cat((dm_voxel, heatmap_joints, heatmap_bbox), dim=1)

        ####################### get prediction ############################

        with torch.no_grad():
            result = snetv6(inputs)
        ...
        if (args.figMode != ''):
            ...
            if (validset == 'evaluation'):
                ...
                if (args.figMode == '2D'):
                    ...
                    ax0.scatter(gt_handMesh_uvd[:, 0], gt_handMesh_uvd[:, 1],gt_handMesh_uvd[:, 2], c=gt_handMesh_uvd[:, 2], s=15)
                    ax0.scatter(gt_objmesh_uvd[:, 0], gt_objmesh_uvd[:, 1],gt_objmesh_uvd[:, 2], c=gt_objmesh_uvd[:, 2], s=15)
                    ax0.title.set_text('Ground Truth')
