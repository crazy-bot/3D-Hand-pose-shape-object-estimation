...
class Ho3DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (not self.isValid):
            ...
            depth_voxel, heatmap_joints, heatmap_bbox,norm_handmesh,norm_objmesh = self.transform.train_transform(sample)
            ################ for testing purpose in visualization ###############
            if (self.preVis):
                self.testVis(depth_voxel,heatmap_joints,heatmap_bbox,norm_handmesh,norm_objmesh)
            ...
            return (depth_voxel, heatmap_joints, heatmap_bbox, norm_handmesh,norm_objmesh )
        else:
            pointcloud = Main_depthmap2points(depth, ux, uy, fx, fy)
            pointcloud = pointcloud.reshape(-1, 3)
            refpoint = Main_pixelToworld(com.reshape(1, -1), ux, uy, fx, fy)
            ...
            voxel44, heatmap_joints, heatmap_bbox = self.transform.val_transform(sample)
            ...
            return (voxel44, heatmap_joints, heatmap_bbox, GT)
...
class V2VVoxelization(object):
    ...
    def val_transform(self, sample):
        points, joints, bbox, refpoint = sample['points'], sample['joints'], \
                                                            sample['bbox'], sample['refpoint']
        ...
        heatmap_bbox = generate_heatmap_gt(bbox, refpoint, new_size, angle, trans, self.sizes, self.d3outputs,
                                           self.pool_factor, self.std)
        return depth_voxel,heatmap_joints,heatmap_bbox
