...
from src.train_helper import train_HO_SNet_v3, val_HO_SNet_v3
from networks.HO_Nets.HO_ShapeNet import HO_ShapeNet
from dataset.HO_Data.HO_shape_v3 import Ho3DDataset


ap = argparse.ArgumentParser()
ap.add_argument("--load_ckpt", type=str, help="relative path to load checkpoint", default='')
ap.add_argument("--save_ckpt", type=str, help="relative path to save checkpoint", default='HO_SNet_v3')
...
print('==> Preparing data .. HO_SNet_v3')
...
net = HO_ShapeNet(input_channels=1+21+8+1,hand_channels=no_handverts,obj_channels=no_objverts)
...
for epoch in range(start_epoch, start_epoch + epochs_num):
    ...
    train_HO_SNet_v3(net, criterion, optimizer, train_loader, device=device)
    val_HO_SNet_v3(net, val_loader, device=device)
