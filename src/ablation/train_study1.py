...
from src.train_helper import train_HO_study1, val_HO_study1
from networks.HO_Nets.HO_study1 import HO_PSNet
from dataset.HO_Data.HO_study1 import Ho3DDataset


ap = argparse.ArgumentParser()
ap.add_argument("--load_ckpt", type=str, help="relative path to load checkpoint", default='')
ap.add_argument("--save_ckpt", type=str, help="relative path to save checkpoint", default='HO_PSNet')
...
old_weights = torch.load(SNet_v2_ckpt)['model_state_dict']
new_weights = {k.replace('module.',''):old_weights[k] for k in list(old_weights.keys()) }
...
for epoch in range(start_epoch, start_epoch + epochs_num):
    ...
    train_HO_study1(net, criterion, optimizer, train_loader, device=device)
    val_HO_study1(net, val_loader, device=device)
